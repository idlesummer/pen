...
`src/pen-core/` (decision layer) and `src/pen-react/runtime/` (runtime layer) are deliberately separate - core has zero React/Ink imports anywhere, produces plain data (a `RenderNode` tree), and every "what if" (unmatched URL, missing slot content) gets resolved *inside* it, provably, before any component ever renders. The runtime layer only translates already-decided data into elements; it never has to guess.
