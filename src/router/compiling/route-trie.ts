...
export type Frame = {
  ...
  error?: string
  default?: string
  slots?: Record<string, TrieNode> // this position's slots, attached to its anchor folder
  paramDepth: number
}
 ...
 *  into the position around them - so a TrieNode is somewhere a URL segment
...
export type TrieNode = {
  ...
  statics?: Record<string, TrieNode>
  dynamic?: TrieNode
  catchall?: TrieNode
  // Rendering
  ...
  // Validation (dropped by sanitizeRouteTrie once diagnostics have run)
  validation?: TrieValidation
}

/** Compile-time bookkeeping, all of it discarded when compilation ends -
 *  which is what lets the route tree be collected. */
type BuildContext = {
  anchorOf: Map<TrieNode, RouteNode>   // position -> the folder that opened it
  positionOf: Map<RouteNode, TrieNode> // folder -> the position it belongs to
  frameOf: Map<RouteNode, Frame>       // folder -> its frame, when it wraps anything
  slotsOf: Map<TrieNode, Record<string, TrieNode>> // position -> the slots declared in it
  pageOwnerOf: Map<TrieNode, RouteNode>
  nodes: TrieNode[]                    // every position, for validation
}
...
function createFrame(routeNode: RouteNode, position: TrieNode): Frame | undefined {
...
function createTrieNode(routeNode: RouteNode, parent: TrieNode, ctx: BuildContext): TrieNode {
...
function openPosition(routeNode: RouteNode, parent: TrieNode, ctx: BuildContext): TrieNode {
  const segment = routeNode.segment

  switch (segment.type) {
    case 'static': {
      const statics = parent.statics ??= dict<TrieNode>()
      return statics[segment.value] ??= createTrieNode(routeNode, parent, ctx)
    }
    case 'dynamic': {
      (parent.validation!.dynamics ??= dict())[segment.value] ??= routeNode
      return parent.dynamic ??= createTrieNode(routeNode, parent, ctx)
    }
    default: {
      (parent.validation!.catchalls ??= []).push(routeNode)
      return parent.catchall ??= createTrieNode(routeNode, parent, ctx)
    }
  }
}
...
function openSlots(routeNode: RouteNode, position: TrieNode, ctx: BuildContext) {
  for (const child of routeNode.children) {
    if (child.segment.type !== 'slot') continue

    const slots = ctx.slotsOf.getOrInsertComputed(position, dict<TrieNode>)
    let slotTrie = slots[child.segment.value]
    if (!slotTrie) { // two folders can declare the same slot name; they merge
      slotTrie = createTrieNode(child, position, ctx)
      slotTrie.depth = 0 // a slot starts a match path of its own
      slots[child.segment.value] = slotTrie
    }
    walk(child, slotTrie, ctx)
  }
}

/** Walks one folder: records the position it belongs to and turns it into a
 *  Frame if it wraps anything, then descends. */
function walk(routeNode: RouteNode, position: TrieNode, ctx: BuildContext) {
...
/** Compiles the route tree into the URL trie. Every render decision that does
 *  not depend on the URL is settled here, so matching never reads a RouteNode
 *  - and the route tree becomes collectable once this returns.
 *
 *  Returns the root position and every position in the trie; the list is for
 *  validation only and is dropped once diagnostics have run. */
export function createRouteTrie(routeTree: RouteNode): [TrieNode, TrieNode[]] {
  const root: TrieNode = {
    urlDepth: 0,
    staticness: 0,
    depth: 0,
