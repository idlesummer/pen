...
export function createSearchTree(routeTree: RouteNode): CompiledSearchTree {
  ...
  traverse(routeTree, {
    visit: (routeNode) => { // the folder's own contribution: does it own this position's page, and/or its default?
      ...
      // A real default always beats an implicit one at the boundary - a later
      // real claim colliding with an earlier one is invalid, which is exactly
      // what conflicts.defaults (below) exists to catch.
