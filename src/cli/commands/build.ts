    ...
    const pipeline = pipe<BuildContext>([
      ...
      },
      {
        ...
        run: async (ctx) => {
          ...
          const code =
            ...
            '\n' +
            'import { RouteManifest } from \'@idlesummer/pen\'\n' +
            '\n' +
      ...
      },
      {
        ...
        run: async (ctx) => {
          ...
          const code = buildComponentMap(ctx.manifest!)
          writeFileSync(componentsTsPath, code, 'utf-8')
        },
      },
      {
        name: 'Writing entry (ts)',
        onSuccess: (_, ctx) => `Saved entry.ts (${format.duration(ctx.duration)})`,
        run: async (ctx) => {
          const genDir = join(ctx.outDir, 'generated')
      ...
      },
      {
        ...
        run: async (ctx) => {
          ...
          const excludedDirs = new Set(['.pen', 'node_modules', 'dist', '.cache'])
