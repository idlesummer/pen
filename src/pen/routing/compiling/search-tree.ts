...
function createSearchNode(anchor: RouteNode, urlPos: number, staticness: number): SearchNode {
  return { anchor, urlPos, staticness, validation: {} }  // Validation is guaranteed to exist here, it's only removed later at sanitization
}
...
function getOrCreateStaticChild(parentSearchNode: SearchNode, childRouteNode: RouteNode): SearchNode {
  ...
  const createStaticChild = () => createSearchNode(childRouteNode, parentSearchNode.urlPos+1, parentSearchNode.staticness)
...
function getOrCreateDynamicChild(parentSearchNode: SearchNode, childRouteNode: RouteNode): SearchNode {
  const dynamicSearchNode = parentSearchNode.dynamic ??= createSearchNode(childRouteNode, parentSearchNode.urlPos+1, parentSearchNode.staticness-1)
...
function getOrCreateSlotChild(parentSearchNode: SearchNode, childRouteNode: RouteNode): SearchNode {
  const slotSearchNodes = parentSearchNode.slots ??= new Map() // slot can't consume url, so it inherits owner's urlPos and staticness
  const slotChildName = childRouteNode.segment.value
  const createSlotChild = () => createSearchNode(childRouteNode, parentSearchNode.urlPos, parentSearchNode.staticness)
